...
describe('canIncludePath', () => {
  ...
  it('should respect a partial wildcard', () => {
    assert.ok(lib.canIncludePath(['includeme/*.txt'], [], 'includeme/text.txt'));
  });
...
describe('dirParseSync', () => {
  ...
  it('should exclude dot files/dirs', () => {
    ...
    assert.deepEqual(lib.parseLocal(['*', '*/**'], ['n_modules/**/*', 'n_modules/**/.*'], rootDir, '/'), { '/': [], src: ['index.js'] });
  });
  it('should traverse test directory', () => {
    ...
    assert.deepEqual(lib.parseLocal(['*'], ['.excludeme/**/*'], rootDir, '/'), exp2);
  });
});
